 ...
 */
@Extension(value = "zookeeper", override = true, order = 1)
public class Curator4ZookeeperRegistry extends Registry {
     ...
     */
    protected Curator4ZookeeperRegistry(RegistryConfig registryConfig) {
     ...
     * 保存服务发布者的url
     */
    private Map<ProviderConfig, List<String>> providerUrls            = new ConcurrentHashMap<>();

    /**
     * 保存服务消费者的url
     */
    private Map<ConsumerConfig, String>       consumerUrls            = new ConcurrentHashMap<>();
    ...
    public synchronized void init() {
        ...
        preferLocalFile = !CommonUtils
            .isFalse(registryConfig.getParameter(PARAM_PREFER_LOCAL_FILE));
     ...
     */
    private static final ConcurrentHashMap<ConsumerConfig, PathChildrenCache> INTERFACE_PROVIDER_CACHE = new ConcurrentHashMap<>();
    ...
    public void unRegister(ProviderConfig config) {
        ...
        if (config.isRegister()) {
            try {
                ...
                if (CommonUtils.isNotEmpty(urls)) {
                    ...
                    if (LOGGER.isInfoEnabled(appName)) {
                        LOGGER.infoWithApp(appName,
                            LogCodes.getLog(LogCodes.INFO_ROUTE_REGISTRY_UNPUB, providerPath, "1"));
                    }
                }
            } catch (Throwable t) {
                if (!RpcRunningState.isShuttingDown()) {
                    throw new SofaRpcRuntimeException(
                        "Failed to unregister provider to zookeeperRegistry!", t);
                }
            }
        }
        // 反订阅配置节点
        if (config.isSubscribe()) {
            try {
                ...
                if (!RpcRunningState.isShuttingDown()) {
                    throw new SofaRpcRuntimeException(
                        "Failed to unsubscribe provider config from zookeeperRegistry!", t);
                }
            }
        }
    }
    ...
    public List<ProviderGroup> subscribe(final ConsumerConfig config) {
        ...
        if (config.isSubscribe()) {
            ...
            try {
                ...
                if (LOGGER.isInfoEnabled(appName)) {
                    LOGGER.infoWithApp(appName,
                        LogCodes.getLog(LogCodes.INFO_ROUTE_REGISTRY_SUB, providerPath));
                }
                ...
                if (pathChildrenCache == null) {
                    ...
                    pathChildrenCache.getListenable().addListener((client1, event) -> {
                        ...
                        switch (event.getType()) {
                            case CHILD_ADDED:
                                providerObserver.addProvider(config, providerPath,
                                    event.getData(), finalPathChildrenCache.getCurrentData());
                                break;
                            case CHILD_REMOVED:
                                providerObserver.removeProvider(config, providerPath,
                                    event.getData(), finalPathChildrenCache.getCurrentData());
                                break;
                            case CHILD_UPDATED:
                                providerObserver.updateProvider(config, providerPath,
                                    event.getData(), finalPathChildrenCache.getCurrentData());
                                break;
                            default:
                                break;
                ...
                List<ProviderInfo> matchProviders = ZookeeperRegistryHelper.matchProviderInfos(
                    config, providerInfos);
                return Collections.singletonList(new ProviderGroup().addAll(matchProviders));
            } catch (Throwable t) {
                throw new SofaRpcRuntimeException(
                    "Failed to subscribe provider from zookeeperRegistry!", t);
            }
        }
        return null;
    }
    ...
    public void unSubscribe(ConsumerConfig config) {
        ...
        if (config.isRegister()) {
            try {
                ...
                if (!RpcRunningState.isShuttingDown()) {
                    throw new SofaRpcRuntimeException(
                        "Failed to unregister consumer to zookeeperRegistry!", t);
                }
            }
        }
        // 反订阅配置节点
        if (config.isSubscribe()) {
            try {
                ...
                if (!RpcRunningState.isShuttingDown()) {
                    throw new SofaRpcRuntimeException(
                        "Failed to unsubscribe provider from zookeeperRegistry!", t);
                }
            }
            try {
                configObserver.removeConfigListener(config);
                ...
                if (!RpcRunningState.isShuttingDown()) {
                    throw new SofaRpcRuntimeException(
                        "Failed to unsubscribe consumer config from zookeeperRegistry!", t);
                }
            }
    ...
    private void closePathChildrenCache(Map<String, PathChildrenCache> map) {
        for (Map.Entry<String, PathChildrenCache> entry : map.entrySet()) {
            try {
                ...
                LOGGER.error("Close PathChildrenCache error!", t);
            }
        }
    }
}